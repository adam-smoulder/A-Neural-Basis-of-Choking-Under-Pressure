...
%     yticks([0 0.05 0.10 0.15])
    ...
    if f == 1
        legend('Random','Aligned','Data','location','nw')
    end
end; clear f
...
%% Try another ANOTHER version that's between the two
% vals_bySubj = [0.6, 0.4, 0.9]; % for 40 bin edges
vals_bySubj = [0.65 0.65 0.65]; % for 30 bin edges

figure
binEdges = linspace(0,90,30); % 0.5deg bins
for f = 1:nsubjects
    val = vals_bySubj(f);
...
%     yticks([0 0.05 0.10 0.15])
    ...
    % Plot the actual value
    plot(wRwDAngle_bySubj(f)*ones(100,1),linspace(0,val,100),'k-','linewidth',1)
    curPrctile = round(sum(wRwDAngle_bySubj(f)>nullAngles_bySubj(f,:))/nnull*100);
%     text(wRwDAngle_bySubj(f),val*0.9647,[num2str(round(wRwDAngle_bySubj(f))) '^o'],...
%         'horizontalalignment','center','fontname','arial','fontsize',12)
%     text(wRwDAngle_bySubj(f),val*0.8824,['(' num2str(curPrctile) '%ile of null)'],...
...
pos = get(gcf,'position');
set(gcf,'position',[702 634 741 200])

% Save it
figname = ['FigS2_MID2angleBetweenTargetPlaneAndRewAxis_rew' rewMethod '_targ' targMethod ]
saveFigAndSvg([figfolder 'recentFigs\'],figname);
saveFigAndSvg([figfolder 'allFigs\'],[figname '_' dateString]);
