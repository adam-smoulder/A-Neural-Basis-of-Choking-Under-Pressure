...
for c = 1:(nclasses-1)
    ...
    % For ease, we'll make X0 a symbolic variable as well that we
    % substitute for each trial. This should be faster than redefining the
    % eq every time. Does require us to make a string for substitution
    % though...
    ...
    % Now define the other variables and equations
    X = sym('X',[ndims 1]);
    lamb = sym('lamb');
