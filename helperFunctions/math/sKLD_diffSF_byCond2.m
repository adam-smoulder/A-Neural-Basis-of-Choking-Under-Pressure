function [sKLD_byCond,sKLD_byNBins_byCond] = sKLD_diffSF_byCond2(input,condLabels,successLabels,nbinsToTry)
...
%
% AS OPPOSED TO THE NORMAL VERSION this combines conditions for the success
% distribution (only for successes), to find how these failure modes for
% specific condition compare to overall successful behaviors
%
% Inputs:
...
%     [~,binEdges] = labelByPercentile(input(OSInds),nbins);
	[balancedInputLabels,binEdges] = labelByPercentile(balancedInput,nbins);
    ...
    succPMF =  histcounts(balancedInputLabels,0.5:nbins+0.5,'normalization','probability');
    sKLD_byNBins_byCond(n,:) = cellfun(@(x) sKLD(x,succPMF),...
        inputLabelPMFs_bySucc_byCond(1,:));
end; clear n
