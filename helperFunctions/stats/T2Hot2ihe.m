function [n,p,T2,X2,v,P] = T2Hot2ihe(X1,X2,alpha)
% HACKED version of the Hotelling's T^2 test described below. All I do is
% take two normal inputs and put them in the format that this function
% desires. Assumes X1 and X2 are N x D (sample x dimension). This should
% only be used after PCA or something like that

X = [[1*ones(size(X1,1),1) X1] ; 2*ones(size(X2,1),1), X2];
