function [decodeAcc_byRew_mean,decodeAcc_byRew_sem] = linearDecodeHell(binnedData,directionLabels,rewardLabels,nfolds,nreps_mean,nreps_sem)
...
%   minimum number of trials is through CV LDA decoding
...
for r = 1:nrewards
    ...
    for i = 1:nreps_mean
        ...
        predDirLabelsSS_byRew_byRep{r,i} = quickerCVLDADecode(repData,repLabels,repFoldLabels);
...
for r = 1:nrewards
    ...
    for rep = 1:nreps_sem
        ...
        for k = 1:nfolds
            ...
            innerPredLabels = quickerCVLDADecode(curInput(innerRepFoldInds(:),:),innerTestLabels,innerFoldLabels(:));
            ...
            %             nbModel = fitcdiscr(trainData,trainLabels); % default for continuous values is gaussian NB; slow mode
