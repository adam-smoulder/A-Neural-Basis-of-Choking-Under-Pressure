function [binnedData] = getSingleDataBinFromTrialData(trialData,binSize,event,delay)
...
%
% Outputs:
% - binnedData: [ntrials x nfactors] matrix containing the binned results
%
% Adam Smoulder, 1/11/21

nfactors = size(trialData(1).neuralData.factorScores,1);
binnedData = nan(length(trialData),nfactors);
for i = 1:length(trialData)
    ...
    if ~isnan(eventTime)
        ...
        binnedData(i,:) = mean(trialData(i).neuralData.factorScores(:,startInd:endInd),2);
    end
end; clear i
end
