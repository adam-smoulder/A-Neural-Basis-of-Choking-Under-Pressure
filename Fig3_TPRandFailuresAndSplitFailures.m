...
pvals_fail_bySubj_byRew = nan(nsubjects,nrewards);
for f = 1:nsubjects
    % Load data
...
%         TPRProjData = TPRProjData+mean(mu_byDir./sigma_byDir);
    ...
    % Split by success and failure; save avgs and zs by reward (add overall
    % failures as a row as well)
    TPRProj_byStatus_byRew = groupDataByLabel(TPRProjData(~skipLabels),[statusLabels(~skipLabels), rewardLabels(~skipLabels)]);
    TPRProj_byStatus_byRew(4,:) = cellfun(@(x,y) [x;y], TPRProj_byStatus_byRew(2,:), TPRProj_byStatus_byRew(3,:), 'uniformoutput', false);
    ...
    TPRProj_byStatus_byRew_median = cellfun(@(x) median(x), TPRProj_byStatus_byRew);
    TPRProj_byStatus_byRew_semed = cellfun(@(x) nansemed(x,1), TPRProj_byStatus_byRew);
    ...
    TPRProj_byStatus_byRew_median_subjSplit{f} = TPRProj_byStatus_byRew_median;
    TPRProj_byStatus_byRew_semed_subjSplit{f} = TPRProj_byStatus_byRew_semed;
    ...
    [~,pvals_fail_bySubj_byRew_tt(f,:)] = cellfun(@(x,y) ttest2(x,y),TPRProj_byStatus_byRew(1,:),TPRProj_byStatus_byRew(4,:));
    ...
    for r = 1:nrewards
        ...
        for b = 1:nboots
            ...
            succ_fail_bootMeans(b) = mean(x(xInds))-mean(a(randi(length(a),[length(a) 1])));
        end; clear b
        ...
        pvals_fail_bySubj_byRew(f,r) = min(sum(succ_fail_bootMeans > 0),sum(succ_fail_bootMeans < 0))/nboots*2; % we check whichever is lower and mult x 2 bc it's 2-sided
    end; clear r
...
% For horizontal jitter, order by success rate level
[~, revorder] = sort(cellArrayToVector(cellfun(@(x) x(1,:), TPRProj_byStatus_byRew_mean_subjSplit, 'uniformoutput', false)), 'descend');
temp = (1:12)';
order(revorder) = temp;
xmods = (order-1) * 0.24/11 - 0.12;
...
p_oshoot = signrank(data(:,1),data(:,2));
p_ushoot = signrank(data(:,1),data(:,3));
...
%% Now make slope for just success and failure
figure
subplot(1,2,1); hold on
...
%     xmod = -0.15+f*0.075;
    for r = 1:nrewards
        ...
        curVals = TPRProj_byStatus_byRew_mean_subjSplit{f}(:,r);
%         curVals = curVals-curVals(1); % set success to 0
        plot((1:2)+xmod,curVals([1 4]),'k-','linewidth',0.5+(pvals_fail_bySubj_byRew(f,r)<0.05)*2,'color',[0.7 0.7 0.7])
        plot(1+xmod,curVals(1),subjShapes(f),'markeredgecolor',rewColors{r},...
            'linewidth',lw,'markersize',ms)
        plot(2+xmod,curVals(4),subjShapes(f),'markeredgecolor',rewColors{r},...
            'linewidth',lw,'markersize',ms)
    end; clear r
end; clear f
% legend(cellfun(@(x) x(1),subjectNames,'uniformoutput',false),'location','NE')
axis([0.5 2.5 minVal maxVal])
xticks(1:2); xticklabels({'Success','Failure'}); xtickangle(22.5)
yticks([-1.5:0.5:1.5])
set(gca,'fontsize',12,'fontname','arial','tickdir','out');
legend(cellfun(@(x) x(1),subjectNames,'uniformoutput',false),'location','SE')
...
set(gcf,'position',[42 516 457 420])

% Save
figname = ['Fig3_TPRandFailure_rewSFCombinedSubj_' method]
saveFigAndSvg([figfolder 'recentFigs\'],figname);
saveFigAndSvg([figfolder 'allFigs\'],[figname '_' dateString]);
...
% [~,p_oshoot,ci_oshoot,stats_oshoot] = ttest(data(:,1),data(:,2));
% [~,p_ushoot,ci_ushoot,stats_ushoot] = ttest(data(:,1),data(:,3));
p_fail = signrank(data(:,1),data(:,4));
disp(['p (succ/fail) = ' num2str(p_fail)])
minVal
maxVal
...
%% See if there's an inverted U in TPA for JUST successes
figure; set(gcf,'position',[-1039 432 495 300])
for f = 1:nsubjects
    subplot(1,nsubjects,f)
    x = 1:4;
    y = TPRProj_byStatus_byRew_mean_subjSplit{f}(1,:);
    err = TPRProj_byStatus_byRew_sem_subjSplit{f}(1,:);
    errorbar(x,y,err,'k-','linewidth',2)
    set(gca,'fontsize',10,'fontname','arial','tickdir','out');
    % set(gca,'xlim',[0.5 4.5])
    axis([0.5 4.5 -0.7 0.4])
    xticks(1:4)
    xticklabels({'S','M','L','J'})
    if f == 1
        ylabel({'Target Preparation Axis','Proj. for Successes (a.u.)'})
    else
        yticklabels([])
    end
...
% Save 
figname = ['Fig3_TPRAndUShoots_JustSuccTPATrend_' method]
saveFigAndSvg([figfolder 'recentFigs\'],figname);
saveFigAndSvg([figfolder 'allFigs\'],[figname '_' dateString]);


for f = 1:nsubjects
    disp(['Monkey ' subjectNames{f}])
    TPA_byRewSF = groupDataByLabel(TPRProj_subjSplit{f}, [rewardLabels_subjSplit{f} statusLabels_subjSplit{f}==1]);
    [~,p_SL] = ttest2(TPA_byRewSF{1,2},TPA_byRewSF{3,2},'vartype','unequal') % success S vs success L
    [~,p_LJ] = ttest2(TPA_byRewSF{3,2},TPA_byRewSF{4,2},'vartype','unequal')
end; clear f
...
%     subplot(1,nsubjects,f)
%     x = 1:4;
%     y = TPRProj_byStatus_byRew_median_subjSplit{f}(1,:);
%     err = TPRProj_byStatus_byRew_semed_subjSplit{f}(1,:);
%     errorbar(x,y,err,'k-','linewidth',2)
% end; clear f

%% What about oshoots?
figure; set(gcf,'position',[-1746 389 918 379])
for f = 1:nsubjects
    subplot(1,nsubjects,f)
    x = 1:4;
    y = TPRProj_byStatus_byRew_mean_subjSplit{f}(2,:);
    err = TPRProj_byStatus_byRew_sem_subjSplit{f}(2,:);
    errorbar(x,y,err,'k-','linewidth',2)
    set(gca,'fontsize',12,'fontname','arial','tickdir','out');
    % set(gca,'xlim',[0.5 4.5])
    axis([0.5 4.5 -0.7 0.4])
    ...
    xticklabels({'S','M','L','J'})
    if f == 1
        ylabel({'Target Preparation Axis','Proj. for OShoots (a.u.)'})
    end
    title(['Monkey ' subjectNames{f}(1)])
end; clear f
...
for f = 1:nsubjects
    subplot(1,nsubjects,f)
    x = 1:4;
    y = TPRProj_byStatus_byRew_mean_subjSplit{f}(3,:);
    err = TPRProj_byStatus_byRew_sem_subjSplit{f}(3,:);
    errorbar(x,y,err,'k-','linewidth',2)
    set(gca,'fontsize',12,'fontname','arial','tickdir','out');
    % set(gca,'xlim',[0.5 4.5])
    axis([0.5 4.5 -1.3 0.3])
    xticks(1:4)
    xticklabels({'S','M','L','J'})
    if f == 1
        ylabel({'Target Preparation Axis','Proj. for UShoots (a.u.)'})
    end
    title(['Monkey ' subjectNames{f}(1)])
end; clear f
